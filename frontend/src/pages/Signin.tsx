...
export default function SignIn() {
  const { signin, signup, error, clearError, loading, isAuthenticated } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();

  // Determine initial mode from URL or default to signin
  ...
  const handleContactNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSignupData(prev => ({ ...prev, contactNumber: value }));
  ...
  const handleSigninSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    clearError();
    const result = await signin(signinData.emailAddress, signinData.password);
    if (result.success) {
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    clearError();
    await signup(signupData.emailAddress, signupData.password, signupData.fullName, signupData.contactNumber);
  };
            ...
            {!isSignup ? (
              ...
              >
                ...
                <Button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 py-3 text-lg bg-white text-black hover:bg-gray-100 mb-4"
                >
                ...
                <div className="relative my-4">
                  ...
                  <div className="relative flex justify-center text-sm">
                    <span className="px-2 bg-[#020617] text-gray-400">Or sign in with email</span>
                  </div>
                </div>
                ...
                <form onSubmit={handleSigninSubmit} className="space-y-4">
                  <Input
                    ...
                    required
                    onChange={(e) => setSigninData(prev => ({ ...prev, emailAddress: e.target.value }))}
                ...
                <form onSubmit={handleSignupSubmit} className="space-y-4">
                  ...
                  <div>
                    <Input
                      ...
                      className={`bg-slate-900/60 border-slate-700 text-white ${
                        contactError ? 'border-red-500' : signupData.contactNumber && !contactError ? 'border-green-500' : ''
                      }`}
                    />
                    {contactError && (
                      <p className="text-red-400 text-xs mt-1">{contactError}</p>
                    )}
                    {signupData.contactNumber && !contactError && (
                  ...
                  <Input
                    ...
                    required
                    onChange={(e) => setSignupData(prev => ({ ...prev, emailAddress: e.target.value }))}
                    className="bg-slate-900/60 border-slate-700 text-white"
                  />
                  <Input
                    type="password"
      ...
      >
        <div className="w-full h-full flex items-center justify-center">
          <DotLottieReact
            ...
            loop
            autoplay
            style={{ width: '90%', height: '90%', maxWidth: '600px' }}
          />
        </div>
      </motion.div>
    </div>
